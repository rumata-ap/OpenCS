...
public class ShellSimplSolverTests
{
    ...
    public void ComputeStripSls_EccentricCompression_ReferenceExample48_SameOrderOfMagnitude()
    {
        ...
        // Ширина раскрытия закреплена нашим же расчётом (книга её не даёт): ψs здесь замыкается
        // по напряжениям (ф. 8.137). Пересчитана 18.09.2026 после введения поправки (8.154):
        // сжимающая N увеличивает высоту сжатой зоны (0,167 → 0,220 м), σs падает с 389 до 272 МПа,
        // acrc — с 0,3177 до 0,2005. Книжная σs = 331,2 МПа получена другим методом (φcrc,
        // табл. 4.2); до правки мы были на 17,5% выше неё, после — на 17,8% ниже, то есть точность
        // относительно книги не изменилась, изменился знак отклонения.
        // 22.09.2026: ф. (8.134) — yc и Ired по центру тяжести приведённого сечения с трещиной,
        // момент перенесён к нему (было yc = x_m и I относительно нейтральной оси). σs = 314,9 МПа
        // против 315,2 по точному упругому расчёту сечения с трещиной и 331,2 в книге (−4,9%);
        // acrc 0,2721 → 0,3078.
        Assert.Equal(0.3078, r.Acrc_mm, 4);
    }
