...
public class ShellSimplWallBaseSectionTests
{
    ...
    //
    // Вуд-Армер OpenCS даёт 1,098 — решает горизонтальная полоса "x, верх" с Nx,des = +508
    // (правило Вуда переносит сдвиг Nxy = 100 в растянутое направление) и Mx,des = 37,2.
    // Вертикаль с обжатием Ny,des = −1300 (граница N − |Nxy|, худшая для η) даёт 0,740.
    // До 21.09.2026 WaMembrane обнулял сжатое направление, вертикаль считалась на чистый
    // изгиб (η = 1,000) и совпадение с эталоном было "не по той причине".
    [Fact]
    ...
    public void WoodArmer_CrackWidths_Regression()
    {
        ...
        // 21.09.2026: с сохранением обжатия вертикали (−850 кН/м) она больше не трещит —
        // решает горизонталь "x, верх": 0,210 / 0,260 (было 0,309 / 0,403 по вертикали
        // как чистому изгибу).
        // 22.09.2026: ф. (8.134) — yc и Ired по центру тяжести сечения с трещиной, момент
        // перенесён к нему. Полоса "x, верх" (M = 24,3, N = +352,8) имеет x_m = 6 мм — ровно
        // в зоне, где прежняя запись (yc = x_m, I относительно нейтральной оси) занижала σs:
        // 264,6 МПа против 316,4 теперь и 314,8 по точному упругому расчёту сечения с трещиной.
        // acrc,long 0,210 → 0,297.
        Assert.InRange(aLong, 0.28, 0.32);
        Assert.InRange(aShort, 0.33, 0.39);
    }
