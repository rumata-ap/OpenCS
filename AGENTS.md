...
OpenSees доступен по фиксированному пути на обоих ПК разработчика. Использовать для задач МКЭ (стержневые элементы, pushover, динамический анализ и т.д.), когда встроенного движка CSfea недостаточно.
...
Запуск команд через stdin:
```powershell
"wipe; model BasicBuilder -ndm 2 -ndf 3; puts 'OpenSees OK'; wipe" | & "C:\Tools\OpenSees\bin\OpenSees.exe"
```

Не искать OpenSees в PATH и не спрашивать путь у пользователя — всегда использовать указанный выше путь.
...
- **`Mx = ∫σx·z dz`, `My = ∫σy·z dz`, `Mxy = ∫τxy·z dz`** (см. `PlateSection.Compute` →
  `Integrate`, `CScore/PlateSection.cs`). `z` — от срединной плоскости вверх (`z>0` = грань `+Z`,
  верх; `PlateRebarLayer.Zsx/Zsy`: «отрицательное — к нижней грани»). Следствие: **положительный
  `Mx`/`My` растягивает грань `+Z` (верх)**, а не низ.
- Конвенция ЛИРА/SCAD для `Mx`/`My`/`Mxy` — **обратная** (растяжение низа при положительном
  моменте). Поэтому импорт усилий (`LiraForceMapper.MapShell`, `ScadXlsForceMapper.MapShell`,
  `LiraApiForceImporter`) инвертирует `Mx`/`My`/`Mxy` по флагу
  `InvertShellBendingMoments` (default `true`) — по аналогии с `InvertBarBendingMoments` для
  стержней. `Nx/Ny/Nxy/Qx/Qy` инверсии не подлежат.
- Инверсия `Mxy` выведена **по аналогии** (тот же интеграл `∫·z dz`, что у `Mx`/`My`), а не
  подтверждена независимо документацией ЛИРА/SCAD — при появлении реального проекта с выраженным
  кручением стоит перепроверить.
...
Спеки и планы не коммитить никогда; оставлять их только локальными файлами в `docs/superpowers/`.
