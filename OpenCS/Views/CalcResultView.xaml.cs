...
namespace OpenCS.Views
{
    ...
    public CalcResultView(CalcResult result, AppViewModel app)
    {
        ...
        {
            Content = new OpenSeesSectionMomentCurvatureResultView(result);
            return;
        }
        ...
        {
            Content = new Sp63CrackWidthResultView(result, task, app);
            return;
        }

        if (task?.Kind == "sp63_deflection")
        {
            Content = new Sp63DeflectionResultView(result, task, app);
            return;
        }
            ...
            or "cracking_batch" or "crack_width_batch" or "total_curvature_batch")
        {
            Content = task.Kind switch
            {
                ...
                "fire_thermal_curvature" => new FireThermalCurvatureResultView(result, app, task),
