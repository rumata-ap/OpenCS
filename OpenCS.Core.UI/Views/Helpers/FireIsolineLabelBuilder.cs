...
public static class FireIsolineLabelBuilder
{
    ...
    public readonly record struct Label(Point2D PositionMm, double AngleDeg, string Text);
    ...
    static List<List<Point2D>> ChainPolylines(IEnumerable<FireIsolineSegment> segments)
    {
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            var chain = new List<Point2D> { list[i].A, list[i].B };
            used[i] = true;
            bool extended;
            do
            {
    ...
    static bool TryAppend(ref List<Point2D> chain, Point2D a, Point2D b)
    {
        if (Near(chain[0], a))
        {
            chain.Insert(0, b);
    ...
    static bool TryPickLabelSpot(List<Point2D> chain, double levelCelsius, out Label label)
    {
        label = default;
        if (chain.Count < 2)
            return false;
    ...
    static bool Near(Point2D a, Point2D b)
