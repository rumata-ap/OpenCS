    ...
    public static IReadOnlyList<FireSectionContourDraw> BuildSectionContours(FireMeshBuildResult meshInfo)
    {
        ...
        foreach (var g in groups)
        {
            ...
            foreach (var loop in ChainBoundaryLoops(edgePairs))
            {
                ...
                var pts = new Point2D[loop.Count];
                for (int i = 0; i < loop.Count; i++)
                {
                    int ni = loop[i];
                    pts[i] = new Point2D(mesh.X[ni] * MmPerM, mesh.Y[ni] * MmPerM);
                }
    ...
    public static IReadOnlyList<FireMeshEdgeDraw> BuildMeshEdges(HeatMesh mesh)
    {
        ...
        void AddEdge(int a, int b)
        {
            ...
            edges.Add(new FireMeshEdgeDraw(
                new Point2D(mesh.X[a] * MmPerM, mesh.Y[a] * MmPerM),
                new Point2D(mesh.X[b] * MmPerM, mesh.Y[b] * MmPerM)));
        }
    }
...
public sealed record FireSectionContourDraw(IReadOnlyList<Point2D> PointsMm, bool IsHole);

/// <summary>Ребро T3-сетки на карте.</summary>
public sealed record FireMeshEdgeDraw(Point2D AMm, Point2D BMm);
