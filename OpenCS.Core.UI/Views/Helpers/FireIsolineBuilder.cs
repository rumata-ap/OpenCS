...
public static class FireIsolineBuilder
{
    ...
    public readonly record struct Segment(Point2D A, Point2D B, double LevelCelsius);
    ...
    static void TryEdgeCrossing(HeatMesh mesh, double[] t, int a, int b, double level, List<Point2D> hits)
    {
        double ta = t[a];
        double tb = t[b];
    ...
    static Point2D Mm(double xM, double yM) => new(xM * 1000.0, yM * 1000.0);
}
