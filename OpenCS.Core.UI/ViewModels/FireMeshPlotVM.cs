...
public sealed record FireTriDraw(
    IReadOnlyList<Point2D> VerticesMm,
...
public sealed class FireMeshPlotVM : ViewModelBase
{
    ...
    void ScheduleInitialBuild()
    {
        ...
        UiServices.Dispatcher.BeginInvoke(() =>
        {
            ScheduleOverlayBuild();
            RebuildGeometry();
        });
    ...
    static IReadOnlyList<ColorBand> BuildBands(double vmin, double vmax)
    {
        ...
        for (int i = 0; i < NumBands; i++)
        {
            ...
            double vm = (v0 + v1) * 0.5;
            var brush = ColormapHelper.GetThermalColor(vm, vmin, vmax).ToHex();
...
public sealed record FireIsolineSegment(Point2D A, Point2D B, double LevelCelsius);
