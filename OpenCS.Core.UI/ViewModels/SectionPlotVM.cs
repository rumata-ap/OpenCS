...
namespace OpenCS.ViewModels
{
    ...
    public record FiberDrawData(
        IReadOnlyList<Point2D> Vertices,  // координаты в мм
        Point2D Centroid,
    ...
    public record NoMeshAreaDrawData(
        IReadOnlyList<Point2D> Hull,                        // мм, CCW
        IReadOnlyList<IReadOnlyList<Point2D>> Holes,        // мм, CW
        IReadOnlyList<(Point2D pt, double val)> HullValues, // значения в вершинах hull
        string Tooltip);
    ...
    /// <summary>Полоса дискретной цветовой шкалы для колорбара (цвет — hex-строка "#AARRGGBB").</summary>
    public record ColorBand(string Brush, string Label);
    ...
    public class SectionPlotVM : ViewModelBase
    {
        ...
        public IReadOnlyList<Point2D>? NeutralAxis { get; }

        /// <summary>Точка максимального сжатия: координаты мм, ε и σ [МПа] (вершина с мин. ε, независимо от режима).</summary>
        public (Point2D Pt, double Eps, double SigMpa)? MaxComprData { get; }
        ...
        public Point2D? NdsCentroid { get; }
            ...
            IReadOnlyDictionary<int, (double StressPa, double Strain)>? recordedFibers = null)
        {
            ...
            Point2D? mcPt = null; double mcEps = double.MaxValue, mcSig = 0;
            ...
            foreach (var (area, ka) in section.EnumerateAreas(k))
            {
                ...
                if (hasMesh)
                {
                    ...
                    foreach (var f in area.Fibers.Where(f => f.TypeFiber != FiberType.point))
                    {
                        ...
                        var centroid  = new Point2D(f.X * 1000, f.Y * 1000);
                    ...
                    if (area.Hull != null && !isRebar)
                    {
                        ...
                        var holePts = area.Holes
                            .Select(h => (IReadOnlyList<Point2D>)ToPointsMm(h.X, h.Y))
                            .ToList();
                ...
                else if (area.Hull != null && !isRebar)
                {
                    ...
                    var hullVals = new List<(Point2D pt, double val)>();
                    for (int i = 0; i < area.Hull.X.Count; i++)
                    {
                        ...
                        hullVals.Add((new Point2D(ex * 1000, ey * 1000), v));
                    }
                    ...
                    for (int yi = 0; yi < nmYs.Count - 1; yi++)
                    {
                        ...
                        var cellPts = (IReadOnlyList<Point2D>)cell
                        ...
                        concrete.Add(new FiberDrawData(cellPts,
                            new Point2D(cx_mm, cy_mm), val, false, cellTip, cellSigMpa, eps_c, cellAmm2)
                ...
                if (area.Hull != null && !isRebar)
                    for (int i = 0; i < area.Hull.X.Count; i++)
                    {
                        ...
                        {
                            mcEps = eps;
                            mcPt  = new Point2D(vx * 1000, vy * 1000);
            ...
            NdsCentroid = ea_c > 1e-6 ? new Point2D(esy_c / ea_c, esz_c / ea_c) : (Point2D?)null;
        ...
        static (IReadOnlyList<Point2D>? outer, IReadOnlyList<IReadOnlyList<Point2D>> holes) ParseWktFull(string? wkt)
        {
            ...
            var pts = new List<Point2D>(xs.Count);
            for (int i = 0; i < xs.Count; i++)
                pts.Add(new Point2D(xs[i] * 1000, ys[i] * 1000));
            var holes = new List<IReadOnlyList<Point2D>>(holeXs.Count);
            for (int h = 0; h < holeXs.Count; h++)
            {
                var hx = holeXs[h]; var hy = holeYs[h];
                if (hx.Count < 3) continue;
                var hpts = new List<Point2D>(hx.Count);
                for (int i = 0; i < hx.Count; i++)
                    hpts.Add(new Point2D(hx[i] * 1000, hy[i] * 1000));
                holes.Add(hpts);
            }
            return (pts, holes);
        }

        static IReadOnlyList<Point2D> ToPointsMm(IList<double> xs, IList<double> ys)
        {
            var pts = new List<Point2D>(xs.Count);
            for (int i = 0; i < xs.Count; i++)
                pts.Add(new Point2D(xs[i] * 1000, ys[i] * 1000));
        ...
        static IReadOnlyList<Point2D>? ComputeNeutralAxisSegment(
            ...
            double xMin, double xMax, double yMin, double yMax)
        {
            ...
            void TryAdd(double x, double y)
            {
                ...
                pts.Add(new Point2D(Math.Clamp(x, xMin, xMax), Math.Clamp(y, yMin, yMax)));
            }
        ...
        static IReadOnlyList<ColorBand> BuildColorBands(double min, double max, bool isRebar)
        {
            ...
            for (int i = NumBands - 1; i >= 0; i--)  // top (max) to bottom (min)
            {
                ...
                string label = $"{val:G4}";
                bands.Add(new ColorBand(color.ToHex(), label));
            }
            return bands;
        }
    }
