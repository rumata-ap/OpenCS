...
public static class FireMeshPlotBuilder
{
    ...
    static List<Point2D> TriVertsMm(HeatMesh mesh, int[] tri)
    {
        var verts = new List<Point2D>(3);
        for (int i = 0; i < 3; i++)
    ...
    static Point2D Mm(double xM, double yM) => new(xM * 1000, yM * 1000);
