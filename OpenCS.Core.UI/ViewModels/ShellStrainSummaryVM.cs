...
public sealed class ShellStrainSummaryVM
{
    ...
    public string  VerdictBrush  { get; } = Argb.FromRgb(128, 128, 128).ToHex();
    ...
    public ShellStrainSummaryVM(CalcResult r)
    {
        ...
        StatusBrush = r.Status == "ok" ? Argb.FromRgb(0, 128, 0).ToHex()
                    : r.Status == "not_converged" ? Argb.FromRgb(255, 140, 0).ToHex()
                    : r.Status == "fail" ? Argb.FromRgb(255, 0, 0).ToHex()
        ...
        try
        {
            ...
            if (root.TryGetProperty("check", out var chk))
            {
                ...
                VerdictBrush = passed ? Argb.FromRgb(0, 128, 0).ToHex() : Argb.FromRgb(255, 0, 0).ToHex();
